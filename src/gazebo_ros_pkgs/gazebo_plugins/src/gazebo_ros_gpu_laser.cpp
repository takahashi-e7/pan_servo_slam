...
void GazeboRosLaser::OnScan(ConstLaserScanStampedPtr &_msg)
{
  ...
  // レンジの95%を超える値はinfにする
  // そうしないとレーザの限界値が全て壁になってしまう
  double limit = laser_msg.range_max * 0.95;
  for( int i = 0; i < laser_msg.ranges.size(); i++ )
  {
    if( laser_msg.ranges[i] > limit ) laser_msg.ranges[i] = DEF_Inf;
  }


  // 距離センサを模す。gazeborosgpuでレーザの本数を
  // 3以下にすると、動かなくなるので ここで一本に間引く
  for( int i = 1; i < laser_msg.ranges.size(); i++ )
  {
    laser_msg.ranges[i] = DEF_Inf;
  }
